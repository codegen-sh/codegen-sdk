...
class PyGenericType(PyNamedType[Parent], GenericType["PyType", Parent], Generic[Parent]):
    ...
    def _get_parameters(self) -> Collection["PyType", Self] | None:
        if self.ts_node_type == "subscript":
            ...
            return Collection(node=self.ts_node, file_node_id=self.file_node_id, G=self.ctx, parent=self, children=types)
            ...
            return Collection(node=type_parameter, file_node_id=self.file_node_id, G=self.ctx, parent=self, children=types)
