...
iconType: "solid"
---
  <img src="/images/import-loops.png" />
...
The steps to identify and fix import loops using Codegen are as follows:
...
- Find strongly connected components using Networkx which are the import loops
```python
G = nx.MultiDiGraph()
...
Not all import cycles are problematic! Here's an example of a cycle that one may think would cause an error but it does not because due to using dynamic imports to break the cycle at runtime:
...
A dynamic import is an import defined inside of a function, method or any executable body of code which delays the import to be executed until that function or method is called.

You can use `imp.is_dynamic` to check if the import is dynamic which allows you to check for imports that are handled more intentionally.
...
<img src="/images/large-import-loop.png" />
...
One common fix to this problem to break this cycle is to move all the shared symbols to a separate `utils.py` file. We can do this using codegen `symbol.move_to_file`:
...
Let's use Codeium to analyze PyTorch's codebase for import cycles. First, we'll set up our analysis:
...
# Load PyTorch codebase
codebase = Codebase.from_repo("pytorch/pytorch")
# If you have PyTorch cloned locally:
# codebase = Codebase("path/to/pytorch")
```

Now we'll create a graph representation of imports and analyze it for cycles:
...
The most problematic import cycles are those that mix static and dynamic imports between the same files. Let's write a function to find these:
