...
```
<Info>
  [codebase.get_file(`path`)](/api-reference/core/Codebase#get_file) returns a [SourceFile](/api-reference/core/SourceFile) if the path points to a source file and a [File](/api-reference/core/File) otherwise.
</Info>
