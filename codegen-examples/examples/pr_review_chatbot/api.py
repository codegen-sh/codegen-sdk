...
def review_pr(pr: PullRequest, gh_token: str) -> str:
    ...
    codebase = Codebase.from_repo(f"{pr.base.repo.full_name}", programming_language=ProgrammingLanguage.PYTHON, tmp_dir="/root")
...
Focus on the most important feedback that will improve code quality."""
    ...
    review = codebase.ai_client.llm_query_with_retry(messages=[{"role": "system", "content": system_prompt}, {"role": "user", "content": user_prompt}], model="gpt-4", max_tokens=1000, temperature=0.3)
